...
X_25d = df_25d.loc[:,['voltage', 'current','soc','temperature']]
y_25d = df_25d.loc[:,['age']]
...
xg_reg = xgb.XGBRegressor(objective ='reg:squarederror', learning_rate = 0.1, gamma=10, min_child_weight=10,
                          subsample = 0.3 ,  colsample_bytree = 0.7 ,eta = 0.1,
...
ax.plot(preds_test_25d_df_loc, color='red', label='Battery age predicted')
plt.plot(y_test_25d_df_loc, color='black', linewidth= 1, label='Battery age actual')
plt.ylabel("age")
