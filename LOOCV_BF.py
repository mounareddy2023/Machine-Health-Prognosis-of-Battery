...
df = df.loc[:,['voltage_min','soc', 'temperature_max','cycle_count','capacity','age']]
...
X = df.loc[:,['voltage_min', 'temperature_max','cycle_count']]
...
xg_reg = xgb.XGBRegressor(objective ='reg:linear', colsample_bytree = 0.3, learning_rate = 0.1,
                 max_depth = 5,gamma=0, alpha = 10, n_estimators = 100)
...
plt.plot( y.iloc[:,0], label="Age-train", color='black')
plt.plot( preds.iloc[:,0], label="Age-pred", color='red')
plt.ylabel("Age")
plt.xlabel("Cycle")
...
plt.figure(figsize = (10,5))
plt.plot(y.iloc[:,1], label="Capacity-train", color='black')
plt.plot(preds.iloc[:,1], label="Capacity-pred", color='red')
plt.ylabel("Capacity")
plt.xlabel("Cycle")
